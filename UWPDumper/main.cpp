...
void OpenTempState()
{
	...
	if (RoGetActivationFactory(
			...
			__uuidof(LauncherStatics), &LauncherStatics) < 0)
	{
...
std::uint32_t __stdcall DumperThread(HMODULE DLLHandle)
{
	...
	std::size_t i = 0;
	auto PackagePathLength = UWP::Current::GetPackagePath().length();
	for (const auto &File : FileList)
	{
		try
		{
			const fs::path WritePath = DumpPath + File.path().wstring().substr(PackagePathLength);
		...
		case DLL_PROCESS_ATTACH:
		{
			...
			if (IPC::GetTargetProcess() == GetCurrentProcessId())
			{
				...
				CreateThread(
					nullptr,
					0,
					(LPTHREAD_START_ROUTINE)DumperThread,
					hDLL,
					0,
					nullptr);
			}
